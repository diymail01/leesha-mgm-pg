...
def add_student(name, phone_number, room_id, rent, deposit, joined_date):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    ...
    if room and room[1] < room[0]:
        cursor.execute('''
            INSERT INTO students (name, phone_number, room_id, monthly_rent, security_deposit, joined_date)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (name, phone_number, room_id, rent, deposit, joined_date))
...
st.title("\U0001F3E0 Royal Homes PG Management System")
create_tables()

menu = st.sidebar.radio("Navigation", [
    "Home", "Add Room", "Add Student", "View Rooms", 
    "View Students", "Update Rent Status", 
    "Student Exit", "Vacant Rooms", "View CSV Files"
])
...
if menu == "Home":
    ...
    joined_date = st.text_input("Joining Date (YYYY-MM-DD)")
    ...
    if st.button("Add Student"):
        if name and phone and joined_date and selected_room:
            add_student(name, phone, room_dict[selected_room], rent, deposit, joined_date)
        else:
    ...
    if show_history:
        query = '''
            SELECT s.student_id, s.name, s.phone_number, r.room_number,
    ...
    if st.button("Export as PDF"):
        ...
        st.download_button(label="\U0001F4C4 Download PDF", data=pdf_bytes, file_name=f"{title}.pdf", mime="application/pdf")
    ...
    if not students.empty:
        student_dict = {f"{row['name']} (ID {row['student_id']})": row['student_id'] for idx, row in students.iterrows()}
        selected = st.selectbox("Select Student", list(student_dict.keys()))
        left_date = st.text_input("Enter Leaving Date (YYYY-MM-DD)", value=str(datetime.today().date()))
...
elif menu == "Vacant Rooms":
    st.subheader("Vacant Rooms")
    conn = sqlite3.connect(DB_NAME)
    df = pd.read_sql_query('''
        SELECT room_number, floor, capacity, current_occupants, 
               (capacity - current_occupants) AS vacancies, monthly_rent 
        FROM rooms 
        WHERE current_occupants < capacity
    ''', conn)
    conn.close()

    if df.empty:
        st.info("No vacant rooms available.")
    else:
        st.dataframe(df)
        if st.button("Export as PDF"):
            pdf_bytes = export_to_pdf(df, "Vacant Rooms Report")
            st.download_button(
                label="\U0001F4C4 Download PDF", 
                data=pdf_bytes, 
                file_name="Vacant_Rooms_Report.pdf", 
                mime="application/pdf"
            )

elif menu == "View CSV Files":
    st.subheader("View CSV Files in Directory")
